...
def login():
    if (
        flask.request.headers.get(app.config["PROXY_AUTH_HEADER"])
        and not "noproxyauth" in flask.request.url
    ):
        return _proxy()
    ...
    if form.validate_on_submit():
        ...
        device_cookie, device_cookie_username = utils.limiter.parse_device_cookie(
            flask.request.cookies.get("rate_limit")
        )
        username = form.email.data
        if not utils.is_app_token(form.pw.data):
            if (
                username != device_cookie_username
                and utils.limiter.should_rate_limit_ip(client_ip)
            ):
                flask.flash("Too many attempts from your IP (rate-limit)", "error")
                return flask.render_template("login.html", form=form, fields=fields)
            if utils.limiter.should_rate_limit_user(
                username, client_ip, device_cookie, device_cookie_username
            ):
        ...
        if user:
            ...
            response.set_cookie(
                "rate_limit",
                utils.limiter.device_cookie(username),
                max_age=31536000,
                path=flask.url_for("sso.login"),
                secure=app.config["SESSION_COOKIE_SECURE"],
                httponly=True,
            )
            flask.current_app.logger.info(
                f"Login succeeded for {username} from {client_ip} pwned={form.pwned.data}."
            )
            ...
            utils.limiter.rate_limit_user(
                username, client_ip, device_cookie, device_cookie_username, form.pw.data
            ) if models.User.get(username) else utils.limiter.rate_limit_ip(
                client_ip, username
            )
            flask.current_app.logger.warn(
                f"Login failed for {username} from {client_ip}."
            )
...
def logout():
    ...
    response = flask.redirect(
        app.config["PROXY_AUTH_LOGOUT_URL"] or flask.url_for(".login")
    )
...
def _proxy():
    ...
    try:
        ...
        flask.current_app.logger.error(
            "Error creating a new user via proxy for %s from %s: %s"
            % (email, client_ip, str(e)),
            e,
        )
        return flask.abort(500, "You don't exist. Go away! (%s)" % email)
    domain = models.Domain.query.get(desireddomain) or flask.abort(
        500, "You don't exist. Go away! (domain=%s)" % desireddomain
    )
    ...
    user.send_welcome()
    flask.current_app.logger.info(
        f"Login succeeded by proxy created user: {user} from {client_ip} through {flask.request.remote_addr}."
    )
    return flask.redirect(url)
