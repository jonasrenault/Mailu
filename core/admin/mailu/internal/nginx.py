...
STATUSES = {
    ...
    "encryption": ("Must issue a STARTTLS command first", {"smtp": "530 5.7.0"}),
    "ratelimit": (
        "Temporary authentication failure (rate-limit)",
        {
            "imap": "LIMIT",
            "smtp": "451 4.3.2",
...
def check_credentials(user, password, ip, protocol=None, auth_port=None):
    if (
        ...
        or (
            protocol == "imap"
            and not user.enable_imap
            and not auth_port in WEBMAIL_PORTS
        )
        or (protocol == "pop3" and not user.enable_pop)
    ):
        return False
    is_ok = False
    # webmails
    ...
    if not is_ok and utils.is_app_token(password):
        for token in user.tokens:
            if token.check_password(password) and (not token.ip or token.ip == ip):
                is_ok = True
                break
    ...
    if method == "none" and protocol == "smtp":
        ...
        if app.config["INBOUND_TLS_ENFORCE"]:
            if "Auth-SSL" in headers and headers["Auth-SSL"] == "on":
                return {"Auth-Status": "OK", "Auth-Server": server, "Auth-Port": port}
            else:
                status, code = get_status(protocol, "encryption")
                return {"Auth-Status": status, "Auth-Error-Code": code, "Auth-Wait": 0}
        else:
            return {"Auth-Status": "OK", "Auth-Server": server, "Auth-Port": port}
    # Authenticated user
    elif method == "plain":
        ...
        try:
            ...
            app.logger.warn(
                f"Received undecodable user/password from nginx: {raw_user_email!r}/{raw_password!r}"
            )
        else:
            try:
                ...
                if check_credentials(
                    user, password, ip, protocol, headers["Auth-Port"]
                ):
